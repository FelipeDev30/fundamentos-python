...
def exibir_artigo(titulo, *args, **kwargs):
    texto = "\n".join(args)
    meta_dados = "\n".join([f"{chave.title()}: {valor}" for chave, valor in kwargs.items()])
    mensagem = f"{titulo}\n\n{texto}\n\n{meta_dados}"
    print(mensagem)
    
exibir_artigo("Estruturas de Dados em Python", "Estruturas de dados são formas de organizar e armazenar dados de maneira eficiente para facilitar o acesso e a manipulação.", "Em Python, existem várias estruturas de dados embutidas, como listas, tuplas, dicionários e conjuntos.", "Cada estrutura de dados tem suas próprias características e usos específicos.", "É importante escolher a estrutura de dados adequada para cada situação para otimizar o desempenho do código.", autor="João Silva", ano=2024, idioma="Português") # Chama a função exibir_artigo, passando o título, o texto do artigo como argumentos posicionais e os metadados como argumentos nomeados. A função formata e exibe a mensagem final.
